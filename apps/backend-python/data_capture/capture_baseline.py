...
# FIX PATHS - This ensures it saves in your project folder
...
print(f"--- SAFETY CAPTURE ACTIVE ---")
print(f"Saving to: {save_path}")
print("Run 'apt update', browse, and SSH now.")
...
def monitor_traffic(packet):
    if packet.haslayer(IP):
        ...
        if forward_key in active_flows:
            ...
            flow_key, is_forward = forward_key, True
            active_flows[flow_key] = {'start_time': now, 'last_time': now, 'fwd_pkts': 0, 'bwd_pkts': 0, 'fwd_bytes': 0,
                                      'bwd_bytes': 0, 'flags': "", 'dport': dport, 'proto': proto}
...
try:
    sniff(prn=monitor_traffic, store=0, filter="not port 22")
    ...
    if len(collected_features) > 0:
        ...
        # 1. Get the exact path of this script (data_capture folder)
        current_dir = os.path.dirname(os.path.abspath(__file__))

        # 2. Go up one level to backend-python
        project_root = os.path.abspath(os.path.join(current_dir, '..'))

        # 3. Target the datasets folder
        datasets_dir = os.path.join(project_root, 'datasets')

        # 4. FORCE Linux to create the folder if it somehow doesn't exist
        ...
        print("\n⚠️ No flows captured. Nothing saved.")
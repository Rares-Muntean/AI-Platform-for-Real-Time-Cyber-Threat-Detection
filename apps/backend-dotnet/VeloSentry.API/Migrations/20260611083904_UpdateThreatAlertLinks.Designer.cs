// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using VeloSentry.API.Database;

#nullable disable

namespace VeloSentry.API.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260611083904_UpdateThreatAlertLinks")]
    partial class UpdateThreatAlertLinks
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.7")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("VeloSentry.API.Database.Models.MonitoredDevice", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("IpAddress")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("LastHeartbeat")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("SshPassword")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("SshUsername")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("MonitoredDevices");
                });

            modelBuilder.Entity("VeloSentry.API.Database.Models.ThreatAlert", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<double>("AnomalyScore")
                        .HasColumnType("double precision");

                    b.Property<string>("DestinationIp")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("DestinationPort")
                        .HasColumnType("integer");

                    b.Property<int>("MonitoredDeviceId")
                        .HasColumnType("integer");

                    b.Property<int>("Protocol")
                        .HasColumnType("integer");

                    b.Property<string>("SourceIp")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("TimeStamp")
                        .HasColumnType("timestamp with time zone");

                    b.Property<double>("TotalPackets")
                        .HasColumnType("double precision");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("MonitoredDeviceId");

                    b.HasIndex("UserId");

                    b.ToTable("ThreatAlerts");
                });

            modelBuilder.Entity("VeloSentry.API.Database.Models.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("VeloSentry.API.Database.Models.MonitoredDevice", b =>
                {
                    b.HasOne("VeloSentry.API.Database.Models.User", "User")
                        .WithMany("MonitoredDevices")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("VeloSentry.API.Database.Models.ThreatAlert", b =>
                {
                    b.HasOne("VeloSentry.API.Database.Models.MonitoredDevice", "MonitoredDevice")
                        .WithMany("ThreatAlerts")
                        .HasForeignKey("MonitoredDeviceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("VeloSentry.API.Database.Models.User", "User")
                        .WithMany("ThreatAlerts")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MonitoredDevice");

                    b.Navigation("User");
                });

            modelBuilder.Entity("VeloSentry.API.Database.Models.MonitoredDevice", b =>
                {
                    b.Navigation("ThreatAlerts");
                });

            modelBuilder.Entity("VeloSentry.API.Database.Models.User", b =>
                {
                    b.Navigation("MonitoredDevices");

                    b.Navigation("ThreatAlerts");
                });
#pragma warning restore 612, 618
        }
    }
}
